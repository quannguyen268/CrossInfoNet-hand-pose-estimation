...
def process(dpt, com ,cube, gt3Dcrop, T, handDetector, importer, rng ):

    dm = norm_dm(dpt, com, cube)  # thermal image
    s = augmentCrop(dm, gt3Dcrop, importer.joint3DToImg(com), cube, T,
                    ['rot', 'sc', 'com', 'none'], handDetector, False, rng=rng)
    resdata = s[0]
    resgt3D = s[2]


    gt3D_ = resgt3D * cube[0] / 2. + com
    jtI_ = transformPoints2D(importer.joints3DToImg(gt3D_), T)

    jtI_ = np.reshape(jtI_, (1, 21 * 3))
    ht_ = joints_heatmap_gen([1], jtI_, (24, 24), points=21)
    hts = np.transpose(ht_, (0, 2, 3, 1)) / 255.

    return resdata, resdata, hts
